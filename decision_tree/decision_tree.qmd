...
In our discussion about logistics regression, we mainly focused on the task of "binary classification" where we are trying to distinguish between two classes of labels (e.g., "spam" or "ham"). In real life though, we often have multi-class labels. For example, the Fashion-MNIST dataset we worked with last week have multiple classes of clothing items. How would we classify this kind of data?

As an example, let's consider a dataset about the iris flowers. This dataset consists of 150 flower measurements from 3 different species: setosa, versicolor, and virginica. For each, we have measurements of the flower's `“petal length”`, `“petal width”`, `“sepal length”`, and `“sepal width”`.
...
Notice when constructing the classifier, we specified `multi_class = 'ovr'`. `ovr` means "one-vs.-rest", which means `sklearn` trains three logistic regression binary classifiers:
...
We can also visualize the decision boundary of this classifier. As we can see, the decision boundaries are still linear, but there are now three lines instead of 1.
...
It turns out there are other (arguably) better models to use when it comes to multi-class classification. Decision trees are one of them.
...
Today, we are going into the territory of non-linear models. There are many non-linear models, decision tree is one of them. You can take more classes about modeling and machine learning to learn about the others.

## Decision Trees, conceptually

What is a decision tree?

A decision tree is simply a tree of questions that must be answered in a sequence to yield a predicted classification. For example, below is a classifier for animals. It starts by asking how many legs the animal has. If the answer is no we immediately say snake. If the answer is four the model is not quite sure, so we must ask an additional question: does it purr?  If the answer is yes, the animal is a cat; and if the answer is no, the animal is a dog. The other possibility is that the number of legs was two, in that case we actually have up to two additional questions we may need to ask before eventually deciding if it is a kangaroo, human, or parrot. Obviously this decision tree does not cover all animals. In principle, with the right sequence of questions, we could build a giant decision tree that could differentiate a vast number of animals.

<img src="images/animal_tree.png" alt='Example of a decision tree' width='600'>

For our iris dataset, we can also come up with a decision tree just by looking at the points. It's important to note in the following decision tree we use multiple features in a single node to make the tree simpler, but in reality decision tree will only make a split based on one feature at a time. We will see an example of an actual tree in the next section.

<img src="images/manual_tree.png" alt='Example of a decision tree on the iris data' width='600'>

Just by looking at the tree, it seems we reached 100% accuracy at least on this dataset. However, this should call for some concerns about potential overfitting.
...
We can use the fitted Decision Tree to predict on four random points in our original data:
...
It looks like this node has three samples yet to be classified, with 1 versicolor, and 2 virginicas, but decision tree stopped here without splitting any further. This results in us classifying the one versicolor wrong.
...
It turns out, because we only used `petal_length` and `petal_width` to fit the decision tree, and these three points have the exact same measurements for `petal_length` and `petal_width`, decision tree was not able to come up with a decision rule to distinguish among these three data points!

In general, decision trees (when unrestricted) will always have a perfect accuracy on the training data, **except** when there are samples from different categories with the exact same features. For example, if the versicolor above had a `petal_length`` of 4.8001, we’d have 100% training accuracy.
...
The decision tree fitted with these two features is very complex, as visualized below. We can see that the tree makes some extremely specific decision rules trying to perfectly capture the dataset.
...
How does this happened? Is there any way we can reduce overfitting? To understand this, we first need to understand how a decision tree is generated.
...
Therefore, the **entropy** $S$ of a node is defined as
...
Intuitively, weighted entropy measures how unpredictable the result of a split is. We want to make the the nodes as predictable as possible, therefore we need to minimize the weighted entropy. In decision trees, the way we minimize the weighted entropy of a split is by trying out different combinations of features and splitting values.
...
Now that we have an idea how decision tree works, let's turn out attention to the issue of overfitting and discuss ways we can avoid or reduce overfitting with decision trees.
...
::: {.columns}
::: {.column width="50%"}
As we've seen earlier, a fully-grown decision tree will almost always overfit data. It has low model bias, but high model variance. In other words, small changes in the dataset will result in very a different decision tree. As an example, the two models below are trained on different subsets of the same data. We can see they come out very differently.
:::
::: {.column width="5%"}
:::
::: {.column width="45%"}
...
The idea of random forest is to harness this variance: build many decision trees and and take the majority vote.
...
- It is not as mathematically nice as some linear models (e.g. OLS and Ridge Regression has a simple closed-form solution).

In practice, people use random forest more often than decision trees because it preforms much better on unseen data in general. It also has the following advantages:
