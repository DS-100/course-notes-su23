...
1. To explain complex phenomena occurring in the world we live in. Examples of this might be:
    ...
    - How is the patients' average height related to the children's average height?
...
3. To measure the causal effects of one event on some other event. For example,
    ...
    This is a much harder question because most statistical tools are designed to infer association not causation. We will not focus on this task in Data 100, but you might take other advanced classes on Causal Inference if you are intrigued! 

Most of the times, we aim to strike a balance between building **interpretable** models and building **accurate models**.
...
For a pair of variables $x$ and $y$ representing our data $\mathcal{D} = \{(x_1, y_1), (x_2, y_2), \dots, (x_n, y_n)\}$, we denote their means/averages as $\bar x$ and $\bar y$, standard deviations as $\sigma_x$ and $\sigma_y$.
...
To convert a variable $x_i$ into standard units, we subtract from it its mean and divide by its standard deviation. For example, $x_i$ in standard units is $\frac{x_i - \bar x}{\sigma_x}$.
